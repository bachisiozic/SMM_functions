genomic_score=function(MATRIX, CLINIC, CLASS = NULL, output_dir = "genomic_score_output", filter_value = 0.1,
  ...
  if (use_known_features) {
    ...
    for (feature in colnames(data)[!colnames(data) %in% c("sample", "pfs_time", "pfs_code", "treatment","disease_stage", "cohort", "imwg", "class", "seq", "radiology")]) {
      ...
      fit_smm=coxph(Surv(pfs_time, pfs_code) ~ feature, data = x[x$disease_stage == "SMM",])
      # Extract hazard ratios for each level of the predictor (sex)
      hr_smm=exp(coef(fit_smm)) 
      # Extract confidence intervals for hazard ratios
      ci_smm=exp(confint(fit_smm))
      result_km_smm=surv_fit(Surv(pfs_time, pfs_code) ~ feature, data = x[x$disease_stage == "SMM",])
      km_p_value_smm=as.numeric(gsub("p = ", "", surv_pvalue(result_km_smm)[,2]))
      pval_smm=km_p_value_smm
      ...
      x.smm=x[x$disease_stage == "SMM",]
      present_smm=sum(x.smm$feature %in% c(1, 2), na.rm=TRUE)
      absent_smm=sum(x.smm$feature %in% c(0), na.rm=TRUE)
      ...
      stat_df=rbind(stat_df, data.frame(
        ...
        HR_low = ci[1],
        HR_high = ci[2],
        p_value_smm=pval_smm,
        HR_smm = hr_smm,
        HR_low_smm = ci_smm[1],
        HR_high_smm = ci_smm[2],
        present = present,
        absent = absent,
        present_MGUS = present_MGUS,
        present_SMM = present_SMM,
        present_smm = present_smm,
        absent_smm = absent_smm,
        FDR = NA
      ))
    }
    ...
    stat_df$NOTE=ifelse(stat_df$NOTE == "." & stat_df$p_value > 0.05 & stat_df$present_smm < min_event_count,"Not enough events in SMM",stat_df$NOTE)
    ...
    stat_df=stat_df[match(c(copynumbers,tumorsuppressorgenes,oncogenesothers,
                            "CNV.Sig","APOBEC","t_CCND1","t_MMSET","t_MAF","CCND3","CCND2","MAFA","MYC","hy","RAS"),
                          stat_df$feature),]
