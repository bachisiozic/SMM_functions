...
# prova=read.delim("~/Desktop/BACHI/Project1/Analysis/JCO_Classification/GenomicClassification_input_matrix_SMM_final.txt",
#                  stringsAsFactors = F)

#### . . . - Running the function - . . . ####
class=SMM_TnT_Cla(GC_input_matrix = "~/Desktop/BACHI/Project1/Analysis/JCO_Classification/GenomicClassification_input_matrix_SMM_final.txt",
                  cyto = "~/Desktop/BACHI/Project1/Analysis/JCO_Classification/new_cyto_SMM_final.txt",
                  assembly = "hg38")
matrix.new=class$matrix

table(class$class$class.new)

colnames(class$class)[2]="class.new.BAC"

data.new=read.delim("~/Desktop/BACHI/Project1/00_FINALdata/2025_tumor_vs_nontumor_03172025.txt",stringsAsFactors = F)
dim(data.new) # 392
dim(class$class) # 392

def=merge(data.new,class$class,by.x="sampleID",by.y="sample")
dim(def)
head(def)
...
############################################################
###########################################################################
clinic=read.delim("~/Desktop/BACHI/Project1/00_FINALdata/clinic_SMM_bac.txt",
                     stringsAsFactors = F)
# matrix=class$matrix
# dim(clinic)
# dim(matrix)
...
source("~/Desktop/BACHI/Project1/GitHub/SMM/SMM_score.R")
source("~/Desktop/BACHI/Project1/GitHub/SMM/crossval_cindex.R")
CLINIC=clinic
CLINIC=CLINIC[complete.cases(CLINIC$pfs_code),]
CLINIC$pfs_code=ifelse(CLINIC$pfs_code == 2,1,CLINIC$pfs_code)
CLINIC=CLINIC[CLINIC$treatment == "no",]
dim(CLINIC)

# pr=merge(CLINIC,CLASS,by="sample")
# dim(pr)
# pr=pr[pr$class.new == "tumor",]
# dim(pr) # 246
# # pr1=pr[!pr$cohort %in% c("mayo","mdacc"),]
# # dim(pr1) # 165
# pr2=pr[pr$cohort %in% c("MSKCC","bolli","Sanger","Moffitt"),]
# dim(pr2) # 165
# 
# table(pr1$disease_stage)
# table(pr2$disease_stage)

MATRIX=class$matrix
MATRIX=MATRIX[MATRIX$sample %in% CLINIC$sample,]
CLASS=class$class
CLASS=CLASS[CLASS$sample %in% CLINIC$sample,]

#genomic_score(MATRIX, CLINIC, output_dir="~/Desktop/BACHI/Project1/GitHub/SMM/SMM_GenomicScore/")

# MATRIX[MATRIX$t_MAF == 1,]
# CLINIC[CLINIC$sample %in% c(MATRIX[MATRIX$t_MAF == 1,"sample"]),]
# CLINIC[CLINIC$sample == "SL225866",]



# genomic_score(MATRIX, 
#               CLINIC, 
#               #CLASS,
#               selection_by = "all",
#               filter_by = "FDR",
#               min_event_count=2,
#               #selection_value = c("MSKCC","mdacc","bolli","NIH-KRD","Sanger","Moffitt","irene"),
#               output_dir="~/Desktop/BACHI/Project1/GitHub/SMM/SMM_GenomicScore/")

prova=genomic_score(MATRIX, 
              CLINIC, 
              CLASS,
              selection_by = "cohort", 
              selection_value = c("MSKCC","bolli","Sanger","Moffitt"), # "MSKCC","bolli","NIH-KRD","Sanger","Moffitt","irene"
              filter_by = "FDR",
              filter_value = 0.1,
              min_event_count=3,
              output_dir="~/Desktop/BACHI/Project1/GitHub/SMM/SMM_GenomicScore/")

crossval_cindex(data=prova$analysis_data,
                output_dir = "~/Desktop/BACHI/Project1/GitHub/SMM/SMM_GenomicScore/",
                fdr_threshold = 0.1)
...
SMM_score=function(MATRIX,CLINIC) {
  
  output=list()
  
  #### . . . - Preparation Data - . . . ####
  if(class(MATRIX)=="character"){
    matrix=read.delim(MATRIX,
                      stringsAsFactors= F, header=T)
  }else{
    matrix=MATRIX
  }
  ...
  if(class(CLINIC)=="character"){
    clinic=read.delim(CLINIC,
                      stringsAsFactors= F, header=T)
  }else{
    clinic=CLINIC
  }
  if(sum(c("sample", "pfs_code", "pfs_time", "disease_stage", "imwg") %in% names(clinic)) != 5){
    stop("ERROR: Please provide the following data columns: sample, pfs_code, pfs_time, disease_stage, imwg")
  }
  ...
  #### . . . - Defining the score - . . . ####
  FINAL_time_KM$FISH_score<- 0
  FINAL_time_KM$FISH_score[FINAL_time_KM$t_MMSET>0 |
                             FINAL_time_KM$t_MAF> 0 |
                             FINAL_time_KM$CNV.SNV_RB1>0 |
                             FINAL_time_KM$CNV_Gain_Amp1q>0]<- 1
  
  FINAL_time_KM$IMWG_fish<- FINAL_time_KM$imwg
  FINAL_time_KM$IMWG_fish[FINAL_time_KM$IMWG_fish =="MGUS"]<-10
  FINAL_time_KM$IMWG_fish<- as.numeric(FINAL_time_KM$IMWG_fish) + FINAL_time_KM$FISH_score
  FINAL_time_KM$IMWG_fish[FINAL_time_KM$IMWG_fish %in% c(10, 11)]<- "MGUS"
  FINAL_time_KM$IMWG_fish[FINAL_time_KM$IMWG_fish %in% c(3,4)]<-"3-4"
  
  
  FINAL_time_KM$imwg2<- FINAL_time_KM$imwg
  FINAL_time_KM$imwg2[FINAL_time_KM$imwg2 %in% c(2,3)]<-"2-3"
